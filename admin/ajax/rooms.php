...
if (isset($_POST['add_room'])) {
    ...
    if (!$room_id) {
        echo json_encode(["status" => "error", "message" => "Failed to insert room"]);
        exit;
    }
...
if (isset($_POST['get_room_images'])) {
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        if($row['thumb']==true){
            ...
            $thumb_btn = "<button class='btn btn-secondary shadow-none' onclick='thumb_image($row[sr_no],$row[room_id])'>
        ...
        echo <<<data
            <tr class="align-middle">
                ...
                <td>$thumb_btn</td>
                <td>
                <button class='btn btn-danger shadow-none' onclick='rem_image($row[sr_no],$row[room_id])'>
                    <i class='bi bi-trash'></i>
                </button>
                </td>
            </tr>
...
if (isset($_POST['thumb_image'])) {
    ...
    // Đặt tất cả ảnh trong phòng về thumb = 0
    ...
    if ($res) {
        ...
        echo json_encode(["success" => false, "error" => "Thumbnail update failed"]);
    }
    exit;
}
...
if (isset($_POST['remove_room'])) {
    ...
    if (!$res1) {
        echo json_encode(["success" => false, "error" => "Failed to select images"]);
        exit;
    }
