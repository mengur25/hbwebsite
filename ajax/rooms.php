...
if (isset($_GET['fetch_rooms'])) {
    // check availability data decdoe
    ...
    if ($chk_avail['checkin'] != '' && $chk_avail['checkout'] != '') {
        ...
        if ($checkin_date == $checkout_date) {
            echo "<h3 class='text-center text-danger'>Check-in and Check-out dates cannot be same</h3>";
            exit;
        } else if ($checkout_date < $checkin_date) {
            echo "<h3 class='text-center text-danger'>Check-in have to be earlier than Check-out dates</h3>";
            exit;
        } else if ($checkin_date < $today_date) {
            echo "<h3 class='text-center text-danger'>Check-in dates earlier than today?</h3>";
            exit;
        }
    }
    ...
    if ($guests['adults'] == 0 && $guests['children'] == 0) {
        echo "<h3 class='text-center text-danger'>Please select at least one guest</h3>";
        exit;
    }

    // facilities data decode
    $facilities_list = json_decode($_GET['facility_list'], true);
    ...
    $oom_res = select("SELECT * FROM `rooms` WHERE `adult`>=? AND `children`>=? AND `status`=? AND `removed`=? ORDER BY `id` DESC", [$adults, $children, 1, 0], 'iiii');
